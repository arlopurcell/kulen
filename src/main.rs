...
fn slice_mesh(mesh: &IndexedMesh) {
    ...
    let max_height = triangles.iter().map(|t| t.points.iter().map(|point| point.z)).flatten().max().unwrap();
    ...
    //for layer in 2..3 {
        ...
        let intersections: Vec<Intersection> = triangles.iter()
...
fn merge_duplicate_intersections(intersects: Vec<Intersection>) -> Vec<Intersection> {
    ...
    for intersect in intersects {
        if merged.contains_key(&intersect.point) {
            merged.insert(intersect.point, merged.get(&intersect.point).unwrap() + intersect.normal);
        } else {
            merged.insert(intersect.point, intersect.normal);
        }
    }

    merged.into_iter().map(|(point, normal)| Intersection{point, normal}).collect()
...
fn polygon_order_points(mut intersects: Vec<Intersection>) -> Vec<Intersection> {
    ...
    let (a, mut b): (Vec<_>, Vec<_>) = intersects.into_iter().partition(|x| is_above(&p.point, &q.point, &x.point));
    b.reverse();
    once(p)
        .chain(a.into_iter())
        .chain(once(q))
...
fn stl_vector_to_nalg_fixed(vertex: &stl_io::Vector<f32>) -> Vector3<I16F16> {
    Vector3::new(vertex[0].to_fixed(), vertex[1].to_fixed(), vertex[2].to_fixed())
}
...
fn triangle_plane_intersection(triangle: &Triangle, plane: &Plane, eps: &I16F16) -> Vec<Intersection> {
    let mut intersections = Vec::new();
    let Triangle{ points: [p1, p2, p3], normal } = triangle;
...
fn segment_plane_intersection(p1: &Vector3<I16F16>, p2: &Vector3<I16F16>, normal: &Vector3<I16F16>, plane: &Plane, eps: &I16F16) -> Vec<Intersection> {
    ...
    if intersection_points.is_empty() && d1 * d2 < 0. {
        ...
        let intersect_point = Vector2::new(round_to_eps(intersect_point.x, eps), round_to_eps(intersect_point.y, eps));
